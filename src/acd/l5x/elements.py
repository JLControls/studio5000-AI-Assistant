...
class Tag(L5xElement):
    ...
    def to_xml(self) -> str:
        ...
        # Operand comments are stored as tag-reference/comment pairs. Preserve
        # them independently from the tag-level description.
        operand_comments = [
            (ref, text)
            for ref, text in self._comments
            if ref not in ("", ".") and text
        ]
        comments_xml = ""
        if operand_comments:
            comment_elements = []
            for operand, text in operand_comments:
                safe_text = self._sanitize_xml_text(text).replace("]]>", "]]]]><![CDATA[>")
                comment_elements.append(
                    f'<Comment Operand="{_escape_xml_attr(operand)}">'
                    f'<![CDATA[{safe_text}]]></Comment>'
                )
            comments_xml = f'<Comments>{"".join(comment_elements)}</Comments>'
...
class ModuleBuilder(L5xElementBuilder):
    ...
    def build(self) -> Module:
        ...
        # Resolve the user description before layout-specific early returns.
        # Drive modules in this v38 project use extended attributes 0x140+
        # rather than 0x01, but their description key is still conventional.
        description = ""
        self._cur.execute(
            "SELECT record_string FROM comments "
            "WHERE parent=? AND member_ref=0 AND object_id=1 LIMIT 1",
        ...
        if len(e1) < 0x30:
            ...
            return Module(
                name, name, "", 0, 0, 0, 0, 0, "Local", 1, "false", major_fault,
...
class TagBuilder(L5xElementBuilder):
    def build(self) -> Tag:
        ...
        def apply_v38_alias(tag: Tag) -> Tag:
            decoded = raw_rec.decode("utf-16le", errors="ignore")
            marker = re.search(r"@\[(\d+)\]", decoded)
            name_match = re.fullmatch(r"(.+?)(\d+)", tag.name)
            if marker and name_match and marker.group(1) == name_match.group(2):
                tag.tag_type = "Alias"
                tag.data_type = None
                tag.alias_for = f"{name_match.group(1)}[{marker.group(1)}]"
            return tag
...
class RoutineBuilder(L5xElementBuilder):
    def build(self) -> Routine:
        ...
        # Fetch rung-level comments from the comments table. Studio v38 uses a
        # routine member reference plus a RegnLink key; older versions use the
        # comment object_id as a 1-based rung index.
        rung_comments: Dict[int, str] = {}
        try:
            comment_parent = (r.comment_id * 0x10000) + r.cip_type
            member_ref = struct.unpack_from("<I", record, 14)[0]
            self._cur.execute(
                "SELECT COUNT(*) FROM regn_links WHERE routine_id=?",
                (self._object_id,),
            )
            has_regn_links = self._cur.fetchone()[0] > 0
            if has_regn_links:
                self._cur.execute(
                    "SELECT c.record_string, rl.rung_id FROM comments c "
                    "JOIN regn_links rl ON rl.routine_id=? "
                    "AND rl.comment_key=((c.rung_content >> 16) & 65535) "
                    "WHERE c.parent=? AND c.record_type=1 "
                    "AND c.rung_content!=0 AND c.member_ref=?",
                    (self._object_id, comment_parent, member_ref),
                )
                rung_index_by_id = {
                    rung_id: index for index, rung_id in enumerate(rung_ids)
                }
                for rec_str, rung_id in self._cur.fetchall():
                    rung_index = rung_index_by_id.get(rung_id)
                    if rung_index is not None and rec_str:
                        rung_comments.setdefault(rung_index, rec_str)
            else:
                self._cur.execute(
...
class ProgramBuilder(L5xElementBuilder):
    ...
    def build(self) -> Program:
        ...
        if main_routine_name is None:
            # v38 may omit the older 0x12D reference. Prefer the conventional
            # main routine name when it exists; otherwise retain no claim.
            main_row = next((row for row in routine_results if row[0] == "MainRoutine"), None)
            if main_row is not None:
                main_routine_name = main_row[0]
...
class TaskBuilder(L5xElementBuilder):
    def build(self, comment_id_to_program: Dict[int, str]) -> Task:
        ...
        if type_val not in _TASK_TYPE_MAP and len(record) > 0x115E:
            v38_type = struct.unpack_from("<H", record, 0x1126)[0]
            if v38_type in _TASK_TYPE_MAP:
                layout_shift = 0x30
                type_val = v38_type
        rate_us = struct.unpack_from("<I", record, 0x106C + layout_shift)[0]
        priority = struct.unpack_from("<H", record, 0x10F8 + layout_shift)[0]
        watchdog_us = struct.unpack_from("<I", record, 0x110A + layout_shift)[0]
        disable_update = record[0x112E + layout_shift]
        ...
        # Studio v38 can retain a stale count that extends beyond the task
        # record.  Never read scheduled-program IDs outside the record.
        prog_count = min(prog_count, max(0, (len(record) - 0x5C) // 4))
        scheduled_programs = []
