...
class ModelGenerator:
    ...
    def __init__(self, tag_descriptions: Optional[dict[str, str]] = None,
                 aoi_params: Optional[dict[str, list[str]]] = None,
                 aoi_id_display: Optional[dict[str, object]] = None):
        self.tag_descriptions = tag_descriptions or {}
        self.aoi_params = aoi_params or {}
        # {AOI_NAME: bilingual-capable display value (html_generator._idj()
        # output) — plain string, or {"en": .., "it": ..} when Italian}. Keyed
        # by the AOI definition name (same identity as aoi_params), used to
        # translate a call block's "head" the same way tag/program/routine
        # identifiers are translated elsewhere.
        self.aoi_id_display = aoi_id_display or {}
    ...
    def _element(self, instr: LadderInstruction) -> dict:
        ...
        # AOI call blocks: "head" is the AOI's own identifier (e.g. "MotorCtl"),
        # so it gets the same bilingual treatment as tag/program/routine
        # identifiers (see html_generator._idj / self.aoi_id_display above).
        # Built-in mnemonics (TON, ADD, JSR, ...) are never Italian, so this
        # lookup is a no-op for every other block type.
        head_display = self.aoi_id_display.get(head, head) if t == InstructionType.AOI else head

        return {'r': 'block', 'head': head_display, 'sub': sub, 'rows': rows,
                'desc': desc, 'tip': tip}
    ...
    def _desc(self, tag: str) -> str:
        ...
        return self._truncate_desc(desc)

    @staticmethod
    def _truncate_desc(desc):
        """Apply the 64-char ellipsis rule to a description.

        `desc` is either a plain string (English-only source) or a bilingual
        {"en": str, "it": str} dict (html_generator._tj() output). ``len()``
        of a dict is its key count, not a character count, so applying the
        plain-string rule directly to a dict silently no-ops — each language
        side must be truncated independently, preserving the dict shape.
        """
        if isinstance(desc, dict):
            return {
                lang: (text[:61] + '…' if isinstance(text, str) and len(text) > 64 else text)
...
def convert_rung_to_model(rung_text: str, rung_number: int = 0, comment: str = "",
                          ...
                          aoi_params: Optional[dict[str, list[str]]] = None,
                          aoi_id_display: Optional[dict[str, object]] = None) -> dict:
    """Parse a rung and return the JSON layout model for the SVG renderer."""
    parser = LadderParser()
    rung = parser.parse_rung(rung_text, rung_number, comment)
    return ModelGenerator(tag_descriptions, aoi_params, aoi_id_display).generate(rung)
