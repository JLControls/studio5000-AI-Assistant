...
This lives in ``tests/`` (not the repo root); pytest discovers it and loads it
before importing any test module under ``tests/``, so plain ``pytest`` works.
``src`` is resolved one level up (``../src``).
