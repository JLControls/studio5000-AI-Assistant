...
2. Run the `status-sync` skill checklist — the commit must include the doc
   deltas the work implies (roadmap/bug status via GitHub Issues + Project — see
   step 8a, superseded annotations, memory)
...
6. **If in-scope status-sync doc deltas (CLAUDE.md, etc.) sit next to
...
8a. **Bug and roadmap tracking (GitHub Issues since 2026-08-10):** Bugs and roadmap
   items live as GitHub Issues + the "Studio 5000 AI Assistant" Project on the
   `JLControls/studio5000-AI-Assistant` fork (the `BUG-NNN`/`FEAT-NNN` id is kept in
   each issue title). If any work touched a bug or feature:
   - Find the issue by its id:
     `gh issue list --repo JLControls/studio5000-AI-Assistant --search "BUG-NNN in:title" --state all --json number,title`
   - Add a work-log comment (date, description, verification results, commit hash(es);
     link related files with markdown):
     `gh issue comment <n> --repo JLControls/studio5000-AI-Assistant --body "<entry>"`
   - When a bug is fixed / a feature ships, close it and set the Project **Status** to
     `Done`: `gh issue close <n> --repo JLControls/studio5000-AI-Assistant`
     (Won't-Fix: also add the `resolution:wont-fix` label.)
   The GitHub issue is now the single source of truth — there are no `docs/BUGS.md`,
   `docs/ROADMAP.md`, or `docs/bugs/` / `docs/roadmap/` files to update.
...
- Reference roadmap context (the GitHub Project / FEAT-NNN issues) where applicable
...
- For each item: suggest a documentation home (a GitHub roadmap issue / Project item,
